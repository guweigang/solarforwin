...
class Solar_Cli_LinkVendor extends Solar_Controller_Command
{
    ...
    protected function _exec($vendor = null)
    {
        ...
        if(file_exists(Solar::$system."/source/{$this->_dashes}/$this->_studly")) {
            ...
            // script/vendor.bat -> ../source/solar/script/solar.bat
            $links[] = array(
                'dir' => "script",
                'tgt' => $this->_dashes.'.bat',
                'src' => "../source/solar/script/solar.bat",
            );
         }

        // if vendor is Solar, make symlink for it
        if('Solar' == $vendor) {
            array_pop($links);
            // remove links for script/vendor
            array_pop($links);
            $test_solar_link = array(
                'dir' => "include/Test",
                'tgt' => $this->_studly.'.php',
                'src' => "../../source/{$this->_dashes}/tests/Test/$this->_studly.php",
            );
            $solar_link = array(
                'dir' => "include",
                'tgt' => 'Solar.php',
                'src' => '../source/solar/Solar.php',
            );
            array_push($links, $solar_link, $test_solar_link);
        }
        $system = Solar::$system;
