...
class Solar_Cli_UnlinkVendor extends Solar_Controller_Command
{
    ...
    protected function _exec($vendor = null)
    {
        ...
        // update by Roy Gu 2010-11-11
        if ('Solar' == $this->_studly) {
            array_shift($links);
            array_shift($links);
            $links[] = "include/{$this->_studly}.php";
            $links[] = "include/Test/{$this->_studly}.php";
        }

        // update by Roy Gu 2010-11-13
        // sync files between include/Vendor <---> source/vendor/Vendor
        if (!Solar::supportSymlink()) {
             $sync_vendor = Solar::factory('Solar_Cli_SyncVendor');
             $sync_vendor->sync($sync_vendor->links($this->_studly, $this->_dashes));
        }
        // sync end.
